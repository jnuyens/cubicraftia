...
#   FUNCTIONAL (5)=6, DECORATIVE (6)=5, MATERIAL_ORE (7)=9, ACCESSORY (8)=5, MOB_DROP (9)=2
#   Total = 50
#
...
func test_all_50_load_and_validate() -> void:
	...
	assert_eq(defs.size(), 66, "BrickRegistry must have exactly 66 BrickDefinitions")
	...
	assert_eq(seen_ids.size(), 66, "All 66 brick_ids must be unique")

# ─── Test 2 ────────────────────────────────────────────────────────────────────

func test_get_by_category_returns_correct_set() -> void:
	var defs: Array = _load_all_from_manifest()
	assert_eq(defs.size(), 66, "Must have 66 definitions to verify category distribution")
	...
	assert_eq(counts.get(BrickDefinition.Category.MOB_DROP,     0), 2, "MOB_DROP must have 2 bricks")
