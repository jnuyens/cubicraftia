...
- D-RECON-01-CORRECTION (added 2026-07-08, plan 08-06; supersedes the last two sentences of D-RECON-01 above): the D-RECON-01 claim that "ShaderWobbleAnimator and the avatar wobble sets remain in use for WILDLIFE fish/ghost ... so ANIM-01 is satisfied via that path" was DISPROVEN by 08-VERIFICATION.md. At the time of that claim the shader-wobble paths were dead code: `_FISH_TINTS = {}` in wildlife.gd left every fish routed to ProceduralCreatureAnimator (not ShaderWobbleAnimator), and the wildlife `kind == "ghost"` branch in _setup_animator was unreachable (ghost is a hostile, not a member of _KIND_TYPE). ANIM-01 was therefore NOT genuinely satisfied when the original entry was written. The 08 gap-closure replan re-wired the real dispatch: plan 08-04 populated _FISH_TINTS (fish_blue #1E69C6 / fish_orange #E8702A / fish_yellow #F2C037) so wildlife fish resolve to ShaderWobbleAnimator, and removed the unreachable wildlife ghost branch; plan 08-05 re-wired the hostile soft-body slime (all 3 tiers) and ghost to ShaderWobbleAnimator in their own _ready(), dropping the _setup_procedural_anim calls. After 08-04 + 08-05, SC1's soft-body set (3 slime tiers + fish + ghost) genuinely animates via the shader-wobble system, and ANIM-01 is now actually true. Sources: 08-04-SUMMARY.md, 08-05-SUMMARY.md, 08-VERIFICATION.md (Gaps Summary).
